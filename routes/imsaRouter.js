...
router.get('/weatherTechSportscarChampionship', async (req, res) => {
    try {
        let seriesData = await homeSchema.find({ seriesName: "IMSA WeatherTech Sportscar Championship" });
        ...
        res.render('imsa', { seriesData, models, homePath });
    }
    catch (error) {
